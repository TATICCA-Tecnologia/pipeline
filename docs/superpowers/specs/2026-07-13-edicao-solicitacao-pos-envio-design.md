...
- **Básico**: só `title` (texto) e `description` (textarea) e `platform`/tipo (select) ficam editáveis — ID, status, prioridade, empresa, cliente, dev, datas de criação/atualização continuam só-leitura aqui (mudam por outros fluxos, não por este form).
